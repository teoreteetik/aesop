...
public class FaultToleranceTest {
    ...
    @Test
    public void faultToleranceTest() throws Throwable {
    ...
    public static class ParallelTest {
        ...
        private static MockSocketAnalyzer faultyAnalyzer;
        private static MockSocketAnalyzer analyzerWithFaultyLIS;
        ...
        private static MockMyLab faultyLIS;
        ...
        public static void setup() {
            ...
            faultyAnalyzer = new MockSocketAnalyzer(system, "127.0.0.1", 50_000, Mode.CLIENT);
            analyzerWithFaultyLIS = new MockSocketAnalyzer(system, "127.0.0.1", 50_001, Mode.CLIENT);
            mockAnalyzer1 = new MockSocketAnalyzer(system, "127.0.0.1", 50_002, Mode.CLIENT);
            mockAnalyzer2 = new MockSocketAnalyzer(system, "127.0.0.1", 50_003, Mode.CLIENT);
            mockAnalyzer3 = new MockSocketAnalyzer(system, "127.0.0.1", 50_004, Mode.CLIENT);

            resultJson = JsonUtil.asJson(
                MyLabResultMsg(
                    Order(
            ...
            Config analyzerConfigWithFaultyLIS = analyzerConfig
                .withValue("MyLabHttpClient.resultUrl", fromAnyRef("http://localhost:8071/result"))
                .withValue("MyLabHttpClient.queryUrl", fromAnyRef("http://localhost:8071/query"));

            Config drivers = ConfigFactory.empty()
                .withValue("FaultyAnalyzer", analyzerConfig.withValue("SocketServer.port", fromAnyRef(50_000)).root())
                .withValue("AnalyzerWithFaultyLIS", analyzerConfigWithFaultyLIS.withValue("SocketServer.port", fromAnyRef(50_001)).root())
        ...
        @Test public void testFaultyAnalyzer() {
            sendInvalidQuery(faultyAnalyzer);
            queryAndReceiveOrder(faultyAnalyzer);
            receiveInvalidResult(faultyAnalyzer);
            receiveResult(faultyAnalyzer);
        }
        @Test public void testAnalyzerWithFaultyLIS()   {
            new JavaTestKit(system) {{
                sendValidQuery(analyzerWithFaultyLIS);
                analyzerWithFaultyLIS.expectNoMsg();
            }};
            receiveResult(analyzerWithFaultyLIS);
        }
        ...
        @Test
        public void validateMessagesReceivedByFaultyLIS() {
            List<String> faultyLisExpectedMsgs = new ArrayList<>();
            faultyLisExpectedMsgs.add(queryJson);
            faultyLisExpectedMsgs.add(resultJson);

            MyLabOrderMsg invalidOrderMsg = MyLabOrderMsg(Order(null, null));
            while (!faultyLisExpectedMsgs.isEmpty()) {
                String receivedMsg = faultyLIS.expectAnyString();
                faultyLisExpectedMsgs.remove(receivedMsg);
                if (receivedMsg.equals(resultJson))
                    faultyLIS.send("");
                else if (receivedMsg.equals(queryJson))
                    faultyLIS.send(invalidOrderMsg);
            }
        }
        ...
        private void sendValidQuery(MockSocketAnalyzer mockAnalyzer) {
            mockAnalyzer
                .send("<ENQ>")
                .expect("<ACK>")
        ...
        private void sendInvalidQuery(MockSocketAnalyzer mockAnalyzer) {
            new JavaTestKit(system) {{
                mockAnalyzer
                    .send("<ENQ>")
                    .expect("<ACK>")
                    .send("<STX>0H|\\^&||PSWD|Harper Labs|2937 Southwestern Avenue^Buffalo^NY^73205||319412-9722||||P|1394-97|19890314121122<CR><ETX>{CS}<CR><LF>")
                    .expect("<ACK>")
                    .send("<STX>1Q<CR><ETX>{CS}<CR><LF>")
                    .expect("<ACK>")
                    .send("<EOT>");
            }};
        }

        private void receiveInvalidResult(MockSocketAnalyzer mockAnalyzer) {
            new JavaTestKit(system) {{
                mockAnalyzer
                    .send("<ENQ>")
                    .expect("<ACK>")
                    .send("<STX>0UNEXPECTED_RECORD_TYPE|\\^&||Password1|Micro1|2937 Southwestern Avenue^Buffalo^NY^73205||319412-9722|LS|1||P|1394-94|19890501074500<CR><ETX>{CS}<CR><LF>")
                    .expect("<ACK>")
                    .send("<STX>5L|1<CR><ETX>4A<CR><LF>")
                    .expect("<ACK>")
                    .send("<EOT>");
